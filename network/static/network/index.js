...
function load_posts(type, page) {
    ...
    document.querySelector('#new_post').onsubmit = () => {
        ...
        .then(result => {
            ...
            // Posted successfully
            if (!result.error) {
                message.innerHTML = `<div class="alert alert-success" role="alert">${result.message}</div>`
            } else {
                message.innerHTML = `<div class="alert alert-danger" role="alert">${result.message}</div>`
            };
        });
        return false;
    };
